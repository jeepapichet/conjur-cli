Feature: Check an environment
  ...
  Background: 
    ...
    And I successfully run `conjur variable create $ns/ssh_private_key PRIVATE_KEY_BODY`
    And I create a new user named "alice@$ns"
    And I successfully run `conjur resource permit variable:$ns/access_key user:alice@$ns execute`
    And I successfully run `conjur resource permit variable:$ns/secret_key user:alice@$ns execute`
    And I login as "alice@$ns"
    And I reset the command list

  Scenario: Check against permitted variables
    When I successfully run `conjur env check --yaml '{ aws_access_key: !var $ns/access_key , aws_secret_key: !var $ns/secret_key }'`
