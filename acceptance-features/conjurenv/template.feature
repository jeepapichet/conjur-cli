Feature: Embed values of Conjur variables into ERB template
  ...
  Background: 
    ...
    And I successfully run `conjur variable create $ns/access_key ABCDEF`
    And I successfully run `conjur variable create $ns/secret_key XYZQWER`
    And I reset the command list

  Scenario:
    When I successfully run `conjur env template --yaml '{ aws_access_key: !var $ns/access_key , aws_secret_key: !var $ns/secret_key }' template.erb `
